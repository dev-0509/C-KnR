...
/*
	Polish Calculator : evaluates expressions by converting them into POSTFIX notations and 
					...
					Scans a single line of Input from the command line and uses get_ch to 
					fetch one character at a time from the line of input read into the buffer
...
int main(int argc, char const * argv[])
{
	...
	getLineIntoBufferFromCmd( argc , argv );
...
void getLineIntoBufferFromCmd(int argc, char const * argv[])
{
	int i = 1, index = 0;
